...
test('完了済みの既存予定は name/status は不変・itemType だけ schedule に補正（updated）', () => {
  const id = externalId('gcal', 'ev1_20260703');
  const doneLegacy = { id, name: '📅 18:00 例会', status: '完了', priority: '🟢 中',
    tags: ['予定'], due: '2026-07-03', memo: '会場A', itemType: 'task', isProject: false,
    subtasks: [], createdAt: NOW, completedAt: NOW, order: 1 };
  const r = mergeExternal({ tasks: [doneLegacy] }, [ev({ name: '別名会' })], 'gcal', NOW);
  const t = r.doc.tasks.find((x) => x.id === id);
  assert.equal(t.itemType, 'schedule'); // itemType は補正
  assert.equal(t.status, '完了');        // status は不変
  assert.equal(t.name, '📅 18:00 例会'); // 完了済みは name も元値保持
  assert.equal(r.updated, 1);
});
